...
int get_size(struct bst_node* bst) {
	if(bst == NULL)
		return 0;
	else
		return (1 + get_size(bst->left) + get_size(bst->right));
...
int bst_size(struct bst* bst) {
	if(bst_isempty(bst) == 1) 
		return 0;
	return (1+get_size(bst->root->left) + get_size(bst->root->right));
}

int get_height(struct bst_node* bst) {
	if (bst == NULL) 
		return 0;
//	left = get_height(bst->left);
//	right = get_height(bst->right);
	if (get_height(bst->left) >= get_height(bst->right))
		return (1+get_height(bst->left));
	else 
		return (1+get_height(bst->right));
...
int get_sum(struct bst_node* bst, int sum) {
	if(bst == NULL) 
		return 0;
	sum -= bst->val;
	if(sum<0)
		return 0;
	if(sum == 0 && bst->left == NULL && bst->right == NULL)
		return 1;
	if(get_sum(bst->left, sum) == 1)
		return 1;
	else 
		return get_sum(bst->right, sum);
